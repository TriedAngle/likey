...
use db::{
    Column, DbBuilder, FullScan, LikePattern, RowId, TrigramIndex, Utf8TableBuilder, execute_like,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stats = execute_like(&col, &mut probe, &like, &mut rows);
    ...
    execute_like(&col, &mut scan, &like, &mut scan_rows);
