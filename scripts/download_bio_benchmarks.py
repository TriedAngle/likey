...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download and prepare DNA/protein benchmark FASTA datasets"
    )
    parser.add_argument("--root", default="data/fasta", help="FASTA output directory")
    ...
    dna_out = root / "dna_benchmark.fna"
    protein_out = root / "protein_benchmark.faa"
    ...
    if protein_out.exists() and not args.force:
        ...
        print(
            f"Wrote protein FASTA: {protein_out} ({bytes_written} bytes, {recs} records)"
        )
