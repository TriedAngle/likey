...
def generate_tpch_tpcds(
    ...
    con.execute(f"EXPORT DATABASE '{out_dir_sql}' (FORMAT CSV, DELIMITER '|', HEADER false)")
    verify_export_headers(con, out_dir, tables, "|", expect_headers=False)
    con.close()
    print(f"{name} export complete: {out_dir}")


def generate_job_duckdb(out_dir: Path, scale_factor: float, force: bool) -> None:
    ensure_dir(out_dir)
...
def main() -> None:
    ...
    parser.add_argument("--root", default="data/raw", help="Raw relational output root")
    ...
    parser.add_argument("--job-sf", type=float, default=1, help="JOB scale factor")
    ...
    parser.add_argument(
        ...
        help="JOB source: download or duckdb",
    )
    ...
    if not args.skip_job:
        if args.job_source == "duckdb":
            generate_job_duckdb(job_dir, args.job_sf, args.force)
        else:
