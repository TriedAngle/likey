...
const PATTERNS: &[(&str, &str)] = &[
    ...
    ("%RGD%", "RGD motif"),
    ("%KDEL", "KDEL suffix"),
    ("%C__C%", "Cys spacing motif"),
    ("%P__P%", "Proline spacing motif"),
    ("%YGGF%", "Peptide motif"),
];

fn main() {
    let data_dir = arg_value("--data-dir").unwrap_or_else(|| "data/benchmarks/protein".to_string());
    ...
    println!("--- Protein Like Benchmark ---");
        ...
        if max_row_bytes.is_some() || remaining_bytes != usize::MAX {
            ...
            for row in table.rows.iter().cloned() {
                ...
                if row_byte_cap != usize::MAX && data.len() > row_byte_cap {
                    ...
                    data = std::str::from_utf8(clipped).expect("protein utf8");
                }
                ...
                if len <= remaining_bytes {
                    ...
                    let partial = std::str::from_utf8(partial).expect("protein utf8");
                    kept.push(Row {
                        id: row.id,
                        desc: row.desc,
                        data: partial,
