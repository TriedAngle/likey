...
- Complexity matters: static is favored by DNA patterns with a selective longer internal anchor; adaptive is favored by common first anchors that quickly fail at later checks; recursive is acceptable on simple or anchored cases but fails catastrophically when multi-`%` DNA patterns create many common one-base branches.
...
- Plots generated by `analysis/generate_matcher_multi_percent_report.py` using matplotlib.
...
![overall_median_geomean.png](figures/overall_median_geomean.png)

![win_counts_by_metric.png](figures/win_counts_by_metric.png)

![winners_by_complexity.png](figures/winners_by_complexity.png)

![specificity_vs_slowdown.png](figures/specificity_vs_slowdown.png)

![edge_case_ratios.png](figures/edge_case_ratios.png)
...
Static is strongest when one literal inside a fixed-width segment is much more selective than earlier short anchors. It can jump directly to that anchor. Adaptive is strongest when the first attempted anchor is common but verification fails at a later, more selective literal; it can switch to the failing anchor within the row. Adaptive is weakest when it repeatedly starts from common first anchors and does not learn across rows. Recursive is weakest when common early `%` branches create many equivalent suffix failures.
...
3. Adaptive anchoring is a robust compromise for natural language text. Quote rows are shorter and contain word-like literals, so adaptive avoids some static misanchors while not paying the extreme recursive backtracking cost seen in DNA.
